...
This repo is a ready-to-use fork of [santifer/career-ops](https://github.com/santifer/career-ops) — no personal data is bundled, just the system. Anyone can clone it, run one command, and have a working job-search command center wired to their own profile.

```bash
git clone https://github.com/deepak-glitch/jobme.git
cd jobme
npm install
npm run init        # Interactive: asks for your name, email, location, target role, comp range.
                    # Creates cv.md, config/profile.yml, portals.yml, modes/_profile.md from templates.
                    # Safe to re-run — existing files are never overwritten.
npm run doctor      # Verifies Node, deps, Playwright
```

Then open Claude Code (or OpenCode / Gemini CLI) in this folder and type `/career-ops` to start. The agent will walk you through filling in `cv.md` and refining your profile.

> Want a non-interactive setup (CI, scripts)? `npm run init -- --yes` accepts all defaults.

---

## Quick Start (Upstream Instructions)

```bash
# 1. Clone and install
git clone https://github.com/deepak-glitch/jobme.git
cd jobme && npm install
npx playwright install chromium   # Required for PDF generation

# 2. One-shot setup (recommended)
npm run init                       # Interactive: walks you through the basics
